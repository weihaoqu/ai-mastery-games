      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 pt-20 sm:pt-24">
        <section className="flex flex-col items-center text-center mb-10">
          <div className="mb-4 p-4 bg-surface-container-highest rounded-2xl shadow-[4px_4px_0_0_#98b67d]">
            <span className="material-symbols-outlined text-primary text-5xl" style={{ fontVariationSettings: "'opsz' 48" }}>sort</span>
          </div>
        ...
        <div className="bg-surface-container-lowest border-2 border-outline-variant rounded-xl p-6 mb-8 shadow-[0_4px_0_0_rgba(152,182,125,1)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm text-on-surface-variant">
            <div className="flex items-start gap-3">
              <span className="material-symbols-outlined text-primary text-xl mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }}>drag_indicator</span>
              <p>{t("rule1")}</p>
            </div>
            <div className="flex items-start gap-3">
              <span className="material-symbols-outlined text-secondary text-xl mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }}>timer</span>
              <p>{t("rule2")}</p>
            </div>
            <div className="flex items-start gap-3">
              <span className="material-symbols-outlined text-tertiary text-xl mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }}>local_fire_department</span>
              <p>{t("rule3")}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
          {difficulties.map((d, i) => (
            ...
            >
              <Link
                href={`/${locale}/tumble/play?difficulty=${d}`}
                onClick={() => track('difficulty_select', { game: 'tumble', difficulty: d })}
