    ...
    <>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 pt-20 sm:pt-24">
        ...
        <section className="flex flex-col items-center text-center mb-10">
          <div className="mb-4 p-4 bg-surface-container-highest rounded-2xl shadow-[4px_4px_0_0_#5b4bb4]">
            <span className="material-symbols-outlined text-tertiary text-5xl" style={{ fontVariationSettings: "'opsz' 48" }} aria-hidden="true">balance</span>
          </div>
        ...
        {/* How to Play */}
        <div className="bg-surface-container-lowest border-2 border-outline-variant rounded-xl p-6 mb-8 shadow-[0_4px_0_0_rgba(91,75,180,0.6)]">
          <h2 className="font-headline text-lg font-bold text-on-surface mb-3 flex items-center gap-2">
            <span className="material-symbols-outlined text-tertiary" aria-hidden="true">help</span>
            {t("howToPlay")}
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm text-on-surface-variant">
            <div className="flex items-start gap-3">
              <span className="material-symbols-outlined text-tertiary text-xl mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }}>menu_book</span>
              <p>{t("rule1")}</p>
            </div>
            <div className="flex items-start gap-3">
            ...
            <div className="flex items-start gap-3">
              <span className="material-symbols-outlined text-secondary text-xl mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }}>tune</span>
              <p>{t("rule3")}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
          {difficulties.map((d, i) => (
            ...
            >
              <Link
                href={`/${locale}/ethics/play?difficulty=${d}`}
                onClick={() => track('difficulty_select', { game: 'ethics', difficulty: d })}
                aria-label={`${tDiff(d)} — ${tDiff(`${d}Desc`)}`}
              >
                <div className="group bg-surface-container-lowest p-5 rounded-xl border-2 border-outline-variant shadow-[0_4px_0_0_rgba(91,75,180,0.5)] hover:shadow-[0_2px_0_0_rgba(91,75,180,0.5)] hover:translate-y-[2px] transition-all cursor-pointer">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="material-symbols-outlined text-tertiary text-xl group-hover:translate-x-1 transition-transform" aria-hidden="true">arrow_forward</span>
                  </div>
                </div>
              </Link>
            </motion.div>
