...
import { getMasteryEmoji } from "@/lib/pipeline/scoring";
...
export default function PipelineResultsPage() {
  ...
  useEffect(() => {
    ...
    const raw = sessionStorage.getItem("pipeline-result");
    if (raw) {
    ...
    try {
      ...
      if (stored) {
        ...
        const sessions = (data.sessions || []).filter((s: SessionResult) => s.game === "pipeline");
        if (sessions.length > 0) { setSession(sessions[sessions.length - 1]); return; }
      }
    } catch { /* fall through */ }
    router.replace(`/${locale}/pipeline`);
  }, [router, locale]);
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12 pt-20 sm:pt-24">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-8">
            <Link href={`/${locale}/pipeline`} className="flex-1">
