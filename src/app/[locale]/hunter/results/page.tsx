...
import { getMasteryEmoji } from "@/lib/hunter/scoring";
...
export default function HunterResultsPage() {
  ...
  useEffect(() => {
    ...
    const raw = sessionStorage.getItem("hunter-result");
    if (raw) {
      try {
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12 pt-20 sm:pt-24">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <div className="bg-surface-container-lowest border-2 border-outline-variant rounded-2xl p-6 sm:p-8 shadow-[0_6px_0_0_rgba(152,182,125,1)] mb-8 text-center">
            ...
            <p className="text-6xl sm:text-7xl font-headline font-extrabold text-primary mb-2">
              {displayScore}
            </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-8">
            <Link href={`/${locale}/hunter`} className="flex-1">
              <button className="w-full px-6 py-3 rounded-xl bg-primary text-on-primary font-bold font-label text-sm shadow-[0_3px_0_0_rgba(0,80,25,1)] hover:shadow-[0_1px_0_0_rgba(0,80,25,1)] hover:translate-y-[2px] transition-all focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary" aria-label={t("playAgain")}>
                {t("playAgain")}
              </button>
            </Link>
            <button
              onClick={() => setCertModalOpen(true)}
              aria-label={tHunter("certificateButton")}
              className="flex-1 px-6 py-3 rounded-xl bg-surface-container border-2 border-outline-variant text-on-surface font-bold font-label text-sm shadow-[0_3px_0_0_rgba(152,182,125,0.5)] hover:shadow-[0_1px_0_0_rgba(152,182,125,0.5)] hover:translate-y-[2px] transition-all focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary flex items-center justify-center gap-2"
            >
              <span className="material-symbols-outlined text-lg">workspace_premium</span>
              {tHunter("certificateButton")}
            </button>
            <ShareButton
              gameName={tGames("hunter.name")}
          ...
          <div className="text-center">
            <Link
              href={`/${locale}`}
              className="text-sm text-on-surface-variant hover:text-primary transition-colors font-label focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary rounded"
            >
              {t("backToHub")} &rarr;
            </Link>
          </div>
